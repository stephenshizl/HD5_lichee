...
int do_boota_linux (struct fastboot_boot_img_hdr *hdr)
{
	...
	if(strlen((const char *)hdr->cmdline)) {
		sprintf((char *)buf, "%d", storage_type);
		strncat((char *)hdr->cmdline, " boot_type=", strlen("boot_type=") + 1);
		strncat((char *)hdr->cmdline, buf, strlen(buf) + 1);
		s = getenv("partitions");
		strncat((char *)hdr->cmdline, " partitions=", strlen("partitions=") + 1);
		strncat((char *)hdr->cmdline, s, strlen(s) + 1);
