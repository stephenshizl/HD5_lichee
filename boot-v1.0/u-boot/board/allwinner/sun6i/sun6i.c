...
void fastboot_partition_init(void) {
	...
	int noused = 0;
	if (storage_type == 2) {
		script_parser_patch("nand0_para", "nand0_used", &noused, 1);
		script_parser_patch("nand1_para", "nand1_used", &noused, 1);
		script_parser_patch("mmc2_para", "sdc_used", &used, 1);
	} else if (storage_type == 0) {
		script_parser_patch("mmc2_para", "sdc_used", &noused, 1);
	}
}
//static struct bootloader_message misc_message;
...
int board_late_init(void)
{
	...
	sprintf(buf, "%d", storage_type);
	setenv("boot_type", buf);
	set_boot_type_arg();
	check_android_misc();
	return 0;
}
