...
static int mmc_config_clock(struct mmc *mmc, unsigned clk)
{
	...
	if (clk <=400000) {
		...
		mmchost->mod_clk = clk;
		if (clk <= 26000000)
			writel(0x01500000 | (n << 16) | m, mmchost->mclkbase);
		else
			writel(0x01500300 | (n << 16) | m, mmchost->mclkbase);
		MMCDBG("mmc %d pllclk %d, mclkbase 0x%x\n",mmchost->mmc_no,pll6clk,readl(mmchost->mclkbase));
	}
	//re-enable mclk
