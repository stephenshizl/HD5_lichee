...
s32 smc_set_card_clk(u32 smc_no, u32 cclk,u32 bus_width,u32 ddr)
{
	...
	if (cclk > 400000) {
		...
		//disable mclk first
		writel(0,mclk_base);
		mmcdbg("mmc %d mclkbase 0x%x\n",mmchost->mmc_no,readl(mclk_base));	
	
		rval =  (src << 24) | (samclk_pha << 20)
	    		| (n << 16) | (outclk_pha << 8) | (m - 1);
		writel(rval, mclk_base);
		mmcdbg("mmc %d mclkbase 0x%x\n",mmchost->mmc_no,readl(mclk_base));
	
		writel(readl(mclk_base)|(1<<31),mclk_base);
		mmcdbg("mmc %d mclkbase 0x%x\n",mmchost->mmc_no,readl(mclk_base));
		...
		if (div > 128) {
			m = 1;
			n = 0;
			mmcinfo("mmc %d Source clock is too high\n",mmchost->mmc_no);
		} else if (div > 64) {
			n = 3;
			m = div >> 3;
		} else if (div > 32) {
	...
	mmcdbg("mmc %d Card clock=%d\n",mmchost->mmc_no,mmchost->mclk);
	//dumphex32("ccmu", (char*)0x01c20000, 0x100);
...
static void mmc_set_ios(struct mmc *mmc)
{
	...
	if (mmc->clock)
		if (mmc_config_clock(mmc)) {
			mmchost->fatal_err = 1;
			mmcinfo("mmc %d Config clock failed\n",mmchost->mmc_no);
			return;
		}
...
out:
	...
	if (error) {
		...
		writel(0x7, &mmchost->reg->gctrl);
		mmc_update_clk(mmc);
		mmcinfo("mmc %d mmc cmd %d err 0x%x\n",mmchost->mmc_no, cmd->cmdidx, error);
	}
