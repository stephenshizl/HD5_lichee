...
int save_user_private_data(char *name, char *data)
{
	...
	char cmp_data_name[8] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};		//用来验证flash 一块区域是否被使用
	...
	if (part_size > 0)
	{
		...
		if (strncmp(user_data_head->magic_name, MAGIC, 5)) {
			...
			//sunxi_flash_write(user_data_offset, USER_DATA_MAXSIZE >> 9, user_data_buffer);	//清空flash,防止flash有其他无用的数据
			printf("init the (user) private space\n");
		}

		if (strncmp(cmp_data_name, (char *)user_data_head, 4)) {			//判断flash 那块区域是否被使用
		...
		if (strncmp(cmp_data_name, (char *)user_data_head, 4)) {			//判断flash 那块区域是否被使用
			if (user_data_head->count > 0) {
				for (i = 0; i < USER_DATA_NUM; i++) {
					user_data_p = (USER_PRIVATE_DATA *)(user_data_buffer + sizeof(USER_DATA_HEAR));
					for (j = 0; j < user_data_head->count; j++) {
						//if (!strcmp(USER_DATA_NAME[i], user_data_p->name) && user_data_p->valid) {					//匹配数据项名称
