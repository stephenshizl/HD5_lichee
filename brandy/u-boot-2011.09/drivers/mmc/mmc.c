...
/*
	...
	if (!(mmc->part_support & PART_SUPPORT)) {
		printf("Card doesn't support boot part\n");
		return 0;
	}


	err = mmc_switch_boot_bus_cond(mmc->control_num, MMC_SWITCH_BOOT_SDR_NORMAL, MMC_SWITCH_BOOT_RST_BUS_COND, MMC_SWITCH_BOOT_BUS_SDRx4_DDRx4);
	if (err) {
		printf("mmc switch boot bus condition failed\n");
		return err;
	}
	printf("mmc switch boot bus condition succeed\n");

	err = mmc_switch_boot_part(mmc->control_num, MMC_SWITCH_PART_BOOT_ACK_ENB, MMC_SWITCH_PART_BOOT_PART_1);
	if (err) {
		printf("mmc switch boot part failed\n");
		return err;
	}
	printf("mmc switch boot part  succeed\n");
	...
	err = mmc_switch_boot_part(mmc->control_num, 0, MMC_SWITCH_PART_BOOT_PART_NONE);	//disable boot mode
	if (err) {
		printf("mmc disable boot mode failed\n");
		return err;
	}
	printf("mmc disable boot mode succeed\n");
