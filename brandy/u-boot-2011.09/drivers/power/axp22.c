...
************************************************************************************************************
*/
s32 axp22_probe_chip_id(u32 chipid[4])
{
		u32 i = 0;
		u8  regaddr[16];
		u8  data[16];
		...
		singledata = 1;
		singleaddr = 0xff;
		axp_i2c_write(AXP22_ADDR, singleaddr, singledata);

		//init pmu regaddr
		for (i = 0; i < 16; i++)
		{
			regaddr[i] = 0x20 + i;
			result |= axp_i2c_read(AXP22_ADDR, regaddr[i], &data[i]);
		}
		...
		for (i = 0; i < 4; i++)
		{
			 ...
			 */
			chipid[0] |= (data[i] << (i * 8));
			chipid[1] |= (data[4 + i] << (i * 8));
			chipid[2] |= (data[8 + i] << (i * 8));
			chipid[3] |= (data[12 + i] << (i * 8));
		}
