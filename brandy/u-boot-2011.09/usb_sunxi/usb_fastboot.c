...
#ifdef SUN6I_FASTBOOT_CONFIG
__le16	device_vendor_id = 0;
__le16	device_product_id =	0;
char manufacturer_name[64] = {0};
...
extern s32 axp_probe_chip_id(u32 chipid[4]);
/*
*******************************************************************************
...
/*
*******************************************************************************
*                     __usb_get_fastboot_id
*
* Description: get fastboot id from sys_config.fex if need
*
* Parameters:
*    void
*
...
*/
#ifdef SUN6I_FASTBOOT_CONFIG
static int __usb_get_fastboot_id(void)
{
	__u32 chipid[4] = {0};

	if( script_parser_fetch("fastboot_feature", "vendor_id", (int *)&device_vendor_id, 1) )
	{
		device_vendor_id = DEVICE_VENDOR_ID;
		printf("get vendor_id failed, used default value=%x\n", device_vendor_id);
	}

	if( script_parser_fetch("fastboot_feature", "mass_storage_id", (int *)&device_product_id, 1) )
	{
		device_product_id = DEVICE_PRODUCT_ID;
		printf("get mass_storage_id failed, used default value=%x\n", device_product_id);
	}

	if( script_parser_fetch("fastboot_feature", "manufacturer_name", (int *)manufacturer_name, 64) )
	{
		printf("get manufacturer_name failed, used default value=%s\n", SUNXI_FASTBOOT_DEVICE_MANUFACTURER);
		memset(manufacturer_name, 0, 64);
		strncpy(manufacturer_name, SUNXI_FASTBOOT_DEVICE_MANUFACTURER, sizeof(manufacturer_name) - 1);
	}

	if( script_parser_fetch("fastboot_feature", "product_name", (int *)product_name, 64) )
	{
		printf("get product_name failed, used default value=%s\n", SUNXI_FASTBOOT_DEVICE_PRODUCT);
		memset(product_name, 0, 64);
		strncpy(product_name, SUNXI_FASTBOOT_DEVICE_PRODUCT, sizeof(product_name) - 1);
	}

	if( script_parser_fetch("serial_feature", "serial_unique", &serial_unique, 1) )
	{
		printf("get serial_unique failed, used default value=%d\n", 0);
		serial_unique = 0;
	}

	if (1 == serial_unique)
	{
		//get the chip id from axp
		if(axp_probe_chip_id(chipid)) //if failed use the default chipid
		{
			memset(serial_number, 0, 256);
			strncpy(serial_number, "20080411", sizeof(serial_number) - 1);
		}
		else   //use axp chip id as fastboot id if get axp chip id success
		{
			chipid[0]=(chipid[0]&0xff000000)>>24;
			chipid[1]=((chipid[1]&0x0f000000)>>4)|(chipid[1]&0x000fffff);
			chipid[2]=((chipid[2]&0xff000000)>>20)|(chipid[2]&0x0000000f);
			memset(serial_number, 0, 256);
			//printf("fastboot_feature:chipid[0]=%x\n", chipid[0]);
			//printf("fastboot_feature:chipid[1]=%x\n", chipid[1]);
			//printf("fastboot_feature:chipid[2]=%x\n", chipid[2]);
			//printf("fastboot_feature:chipid[3]=%x\n", chipid[3]);
			sprintf(serial_number, "%02x%06x%03x%08x", chipid[0],chipid[1],chipid[2],chipid[3]);
		}
	}
	else
	{
		if( script_parser_fetch("fastboot_feature", "serial_number", (int *)serial_number, 256) )
		{
			printf("get serial_number failed, used default value=%s\n", SUNXI_FASTBOOT_DEVICE_SERIAL_NUMBER);
			memset(serial_number, 0, 256);
			strncpy(serial_number, SUNXI_FASTBOOT_DEVICE_SERIAL_NUMBER, sizeof(serial_number) - 1);
		}
	}
	printf("fastboot_feature:device_vendor_id=%x\n", device_vendor_id);
	printf("fastboot_feature:device_product_id=%x\n", device_product_id);
	printf("fastboot_feature:manufacturer_name=%s\n", manufacturer_name);
	printf("fastboot_feature:product_name=%s\n", product_name);
	printf("fastboot_feature:serial_number=%s\n", serial_number);
	printf("fastboot_feature:serial_unique=%d\n", serial_unique);
	printf("get fastboot config success\n");
	return 0;
}
#endif
