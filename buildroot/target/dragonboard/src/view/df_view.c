...
static struct df_window_menu    video_window_menu;
...
static struct df_window         gps_window;
static struct df_window_menu    gps_window_menu;

static struct df_window         mic1_window;
static struct df_window_menu    mic1_window_menu;

static struct df_window         mic2_window;
static struct df_window_menu    mic2_window_menu;
static int loop_mode_on = 1;
...
static struct df_button         clear_button;
static struct df_button         switch_button;
static struct df_button         loop_ctl_button;
...
static int auto_window_init(void)
{
    ...
    auto_window.desc.posx   = manual_window.desc.width;
   // auto_window.desc.posy   = (layer_config.height >> 1) + height_adjust;
//    auto_window.desc.posy   = manual_window.desc.height;
   auto_window.desc.posy   = 0;
    if(mic_activated){
//         auto_window.desc.width  = (layer_config.width >> 2)-(layer_config.width>>MIC_POWER_BAR_WITH_SHIFT);
         auto_window.desc.width  = layer_config.width >> 2;
    }
    else
    {
         auto_window.desc.width  = layer_config.width >> 2;
...
static int manual_window_init(void)
{
    ...
    if(mic_activated){
        manual_window.desc.width  = (layer_config.width >> 2)-(layer_config.width>>MIC_POWER_BAR_WITH_SHIFT);
    }
    else
    {
         manual_window.desc.width  = layer_config.width >> 2;
    }
...
static int loop_ctl_button_init(void)
{
	int pipe_fd;
	int ret = -1;
	int mode_on = 1;
	db_debug("loop_ctl_button_init...\n");
	strcpy(loop_ctl_button.name, LOOP_CTL_BUTTON_NAME);
	loop_ctl_button.width  = BUTTON_WIDTH;
	loop_ctl_button.height = BUTTON_HEIGHT;
	loop_ctl_button.x      = (misc_window.desc.width - switch_button.width) / 2 + BUTTON_WIDTH;
	loop_ctl_button.y      = 0;

	loop_ctl_button.bdcolor = COLOR_RED_IDX;
	loop_ctl_button.bgcolor = COLOR_BLUE_IDX;
	loop_ctl_button.fgcolor = COLOR_WHITE_IDX;
	DFBCHECK(misc_window.surface->SetColor(misc_window.surface, 
	                                       color_table[loop_ctl_button.bdcolor].r, 
	                                       color_table[loop_ctl_button.bdcolor].g, 
	                                       color_table[loop_ctl_button.bdcolor].b, 0xff));
	DFBCHECK(misc_window.surface->DrawLine(misc_window.surface, loop_ctl_button.x, 
	                                                            loop_ctl_button.y,
	                                                            loop_ctl_button.x,
	                                                            loop_ctl_button.y + loop_ctl_button.height - 1));
	DFBCHECK(misc_window.surface->DrawLine(misc_window.surface, loop_ctl_button.x, 
	                                                            loop_ctl_button.y,
	                                                            loop_ctl_button.x + loop_ctl_button.width - 1,
	                                                            loop_ctl_button.y));
	DFBCHECK(misc_window.surface->DrawLine(misc_window.surface, loop_ctl_button.x + loop_ctl_button.width - 1, 
	                                                            loop_ctl_button.y,
	                                                            loop_ctl_button.x + loop_ctl_button.width - 1,
	                                                            loop_ctl_button.y + loop_ctl_button.height - 1));
	DFBCHECK(misc_window.surface->DrawLine(misc_window.surface, loop_ctl_button.x, 
	...
	DFBCHECK(misc_window.surface->SetColor(misc_window.surface, 
	                                       color_table[loop_ctl_button.bgcolor].r, 
	                                       color_table[loop_ctl_button.bgcolor].g, 
	                                       color_table[loop_ctl_button.bgcolor].b, 0xff));
	DFBCHECK(misc_window.surface->FillRectangle(misc_window.surface, loop_ctl_button.x + 1,
	                                                                 loop_ctl_button.y + 1,
	                                                                 loop_ctl_button.width - 2,
	                                                                 loop_ctl_button.height - 2));

	DFBCHECK(misc_window.surface->SetFont(misc_window.surface, font24));
	DFBCHECK(misc_window.surface->SetColor(misc_window.surface, 
	                                       color_table[loop_ctl_button.fgcolor].r, 
	                                       color_table[loop_ctl_button.fgcolor].g, 
	                                       color_table[loop_ctl_button.fgcolor].b, 0xff));
	DFBCHECK(misc_window.surface->DrawString(misc_window.surface, loop_ctl_button.name, -1, loop_ctl_button.x + (loop_ctl_button.width >> 1),
	            MENU_HEIGHT / 2 + FONT24_HEIGHT / 2 - FONT24_HEIGHT / 8, DSTF_CENTER));

	misc_window.surface->Flip(misc_window.surface, NULL, 0);

	unlink(LOOP_CTL_PIPE_NAME);
	if (mkfifo(LOOP_CTL_PIPE_NAME,S_IFIFO | 0666) == -1) {
...
static int button_redraw(int color,struct df_button button)
{
    DFBCHECK(misc_window.surface->SetColor(misc_window.surface, 
                                           color_table[color].r, 
                                           color_table[color].g, 
                                           color_table[color].b, 0xff));
    DFBCHECK(misc_window.surface->FillRectangle(misc_window.surface, button.x + 1,
                                                                     button.y + 1,
                                                                     button.width - 2,
                                                                     button.height - 2));
    ...
    DFBCHECK(misc_window.surface->SetFont(misc_window.surface, font24));
    DFBCHECK(misc_window.surface->SetColor(misc_window.surface, 
                                           color_table[button.fgcolor].r, 
                                           color_table[button.fgcolor].g, 
                                           color_table[button.fgcolor].b, 0xff));
    DFBCHECK(misc_window.surface->DrawString(misc_window.surface, button.name, -1, button.x + (button.width >> 1),
                MENU_HEIGHT / 2 + FONT24_HEIGHT / 2 - FONT24_HEIGHT / 8, DSTF_CENTER));
...
//     wifi_window.desc.width  = (layer_config.width >> 1)-(layer_config.width>>MIC_POWER_BAR_WITH_SHIFT);
     wifi_window.desc.width  = layer_config.width >> 2;
    }else
    {
...
static int gps_window_init(void)
{
	db_debug("gps list window init...\n");
	gps_window.desc.flags  = (DWDESC_POSX | DWDESC_POSY | DWDESC_WIDTH | DWDESC_HEIGHT | 
	                             DWDESC_CAPS);
	if(mic_activated){
		 gps_window.desc.posx	 = (layer_config.width >> 1)+(layer_config.width>>MIC_POWER_BAR_WITH_SHIFT) 
							+ wifi_window.desc.width;
		//	   wifi_window.desc.width  = (layer_config.width >> 1)-(layer_config.width>>MIC_POWER_BAR_WITH_SHIFT);
		 gps_window.desc.width  = layer_config.width >> 2;
	}else{
		 gps_window.desc.posx	 = (layer_config.width >> 1) + wifi_window.desc.width;
		 gps_window.desc.width  = (layer_config.width >> 2);
	}
	
	if(camera_activated){
		gps_window.desc.posy   = layer_config.height >> 1;
		gps_window.desc.height = (layer_config.height >> 1);
	}else{
		gps_window.desc.posy   = 0;
		gps_window.desc.height = layer_config.height;
	}
	gps_window.desc.caps   = DWCAPS_ALPHACHANNEL;
	
	DFBCHECK(layer->CreateWindow(layer, &gps_window.desc, &gps_window.window));
	gps_window.window->GetSurface(gps_window.window, &gps_window.surface);
	gps_window.window->SetOpacity(gps_window.window, 0xff);
	gps_window.window->SetOptions(gps_window.window, DWOP_KEEP_POSITION);

	gps_window.bgcolor = item_init_bgcolor;
	DFBCHECK(gps_window.surface->SetColor(gps_window.surface, 
	                                         color_table[gps_window.bgcolor].r, 
	                                         color_table[gps_window.bgcolor].g, 
	                                         color_table[gps_window.bgcolor].b, 0xff));
	DFBCHECK(gps_window.surface->FillRectangle(gps_window.surface, 0, 0, 
	            gps_window.desc.width, gps_window.desc.height));
	
	/* draw menu */
	if (script_fetch("df_view", "gps_menu_name", (int *)gps_window_menu.name, 8)) {
		strcpy(gps_window_menu.name, GPS_MENU_NAME);
	}
	gps_window_menu.width  = gps_window.desc.width;
	gps_window_menu.height = MENU_HEIGHT;
	
	if (script_fetch("df_view", "menu_bgcolor", &gps_window_menu.bgcolor, 1) || 
	        gps_window_menu.bgcolor < COLOR_WHITE_IDX || gps_window_menu.bgcolor > COLOR_BLACK_IDX) {
		gps_window_menu.bgcolor = COLOR_YELLOW_IDX;
	}

	if (script_fetch("df_view", "menu_fgcolor", &gps_window_menu.fgcolor, 1) || 
	        gps_window_menu.fgcolor < COLOR_WHITE_IDX || gps_window_menu.fgcolor > COLOR_BLACK_IDX) {
		gps_window_menu.fgcolor = COLOR_BLACK_IDX;
	}

	gps_window_menu.bgcolor=COLOR_BEAUTY_IDX;
	gps_window_menu.fgcolor=COLOR_WHITE_IDX;
	DFBCHECK(gps_window.surface->SetColor(gps_window.surface, 
										 color_table[gps_window_menu.bgcolor].r, 
										 color_table[gps_window_menu.bgcolor].g, 
										 color_table[gps_window_menu.bgcolor].b, 0xff));
	DFBCHECK(gps_window.surface->FillRectangle(gps_window.surface, 0, 0, gps_window_menu.width, gps_window_menu.height));
	DFBCHECK(gps_window.surface->SetFont(gps_window.surface, font24));
	DFBCHECK(gps_window.surface->SetColor(gps_window.surface, 
										 color_table[gps_window_menu.fgcolor].r,
										 color_table[gps_window_menu.fgcolor].g, 
										 color_table[gps_window_menu.fgcolor].b, 0xff));
	DFBCHECK(gps_window.surface->DrawString(gps_window.surface, gps_window_menu.name, -1, 4, 
			MENU_HEIGHT / 2 + FONT24_HEIGHT / 2 - FONT24_HEIGHT / 8, DSTF_LEFT));

	gps_window.surface->DrawLine(gps_window.surface,0,0,0,gps_window.desc.height);
	gps_window.surface->Flip(gps_window.surface, NULL, 0);
	return 0;
}
...
static int df_windows_init(void)
{
    ...
    if(disp_output_type==DISP_OUTPUT_TYPE_HDMI){
        ...
        if(0==script_fetch("df_view", "tv_scale_factor", &tv_scale_factor,1)){
            if(tv_scale_factor<50&&tv_scale_factor>100){        
             ...
             }else
             {
                ...
                ioctl(disp, DISP_CMD_LAYER_GET_PARA,  (void*)args);    
...
static void button_handle(int x, int y)
{
	...
	if (x > clear_button.x && x < clear_button.x + clear_button.width &&
		...
		if (switch_press) {
			switch_press = 0;
			switch_camera();
			button_redraw(switch_button.bgcolor,switch_button);
		}
		else {
			switch_press = 1;
			button_redraw(COLOR_CYAN_IDX,switch_button);
		}
	}
	else if(x > (misc_window.desc.posx + loop_ctl_button.x) && 
         x < (misc_window.desc.posx + loop_ctl_button.x + loop_ctl_button.width) && 
         y > (misc_window.desc.posy + loop_ctl_button.y) && 
         y < (misc_window.desc.posy + loop_ctl_button.y + loop_ctl_button.height)){
		if (loop_ctl_press) {
			loop_ctl_pipe_fd = open(LOOP_CTL_PIPE_NAME, O_WRONLY |O_NONBLOCK,0);						
			if(loop_ctl_pipe_fd==NULL){
				db_error("fail to open loop_ctl_pipe\n");
			}
			ret = write(loop_ctl_pipe_fd, &loop_mode_on, sizeof(loop_mode_on));
			if(ret == -1){
				db_error("write loop pipe failed\n");
			}
			...
			button_redraw(loop_ctl_button.bgcolor,loop_ctl_button);
		}
		else {
			loop_ctl_press = 1;
/*			loop_ctl_pipe_fd = open(LOOP_CTL_PIPE_NAME, O_WRONLY |O_NONBLOCK,0);						
			if(loop_ctl_pipe_fd==NULL){
				printf("mic1tester:fail to open loop_ctl_pipe\n");
			}
			db_msg("LOOP ON\n");
			ret = write(loop_ctl_pipe_fd, &loop_ctl_press, sizeof(loop_ctl_press));
			if(ret == -1){
				printf("write loop pipe failed\n");
			}
			close(loop_ctl_pipe_fd);*/
			button_redraw(COLOR_CYAN_IDX,loop_ctl_button);
		}
	} 
	else{
		switch_press = 0;
